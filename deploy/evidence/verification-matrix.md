...
| Demo implementation contract | `deploy/demo/contract-tests.ps1`（亦由 static gate 调用） | PowerShell 7；无需 Docker/SQL/HTTP/E2E/browser | 临时 profiles/maps（默认清理） | owner/namespace/wait/loopback/attestation/map 拒绝、零碰撞 preflight、secret finally、owner-tuple teardown、全 Draft evidence 通过 | PASS：70 assertions；仅实现合同 |
