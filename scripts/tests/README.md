...
### `t12/` — operation-log + availability-cache + notifications (`add-supporting-capabilities` §1–§3 backend slices)

Entry: `pwsh scripts/tests/t12/run.ps1 [Check|List|OperationLog|Cache|Notifications|Unit]`
(default `Check`). Maven modes are narrow Surefire selections from
`booking-api/`; no `verify`, no aggregation into the full build.

- `Check` (default): static only — verifies all three main/test trees
  (`com/yu030x/booking/{log,cache,notification}`) exist and hold sources,
  runs `git diff --check`, and aborts on local drift inside strongly forbidden
  shared paths (pom.xml, `src/main/resources`, `sql/`, `deploy/`, `docs/`,
  `booking-web/`). Other local drift lines are printed as informational.
- `List`: prints the complete test inventory of the three slices without
  executing anything.
...
- `Unit`: union selection of all three slice patterns in one surefire run:
  `-Dtest=com.yu030x.booking.log.**,com.yu030x.booking.cache.**,com.yu030x.booking.notification.**`.
