...
- Render a pending-only approvals list with selected-item detail sourced only from the returned `BookingView` and approve/reject responses, backed by T09's exact endpoints. `GET /api/v1/admin/approvals?pageNumber&pageSize` is ordered by `createdAt ASC,id ASC`; there is no separate approval-detail endpoint. The workflow includes bounded Unicode-code-point comments, two-step confirmation, request de-duplication, and server-truth list refreshes.
...
- Explicitly gate implementation on the T04 and T09 sibling handoffs. The gate was satisfied by merging T09 commit `d81578b` into this worktree in `a7c6ce2` and rereading the authoritative backend. ADMIN-only endpoints return `40300` for non-ADMIN callers; approvals are pending-only and ordered `createdAt ASC,id ASC`; the list returns `PageResult<BookingView>` and no separate approval-detail transport exists; approve/reject use `POST /api/v1/admin/bookings/{id}/approve` and `/reject` and return `BookingView`; approve comments trim with blank→`null` and allow at most 500 Unicode code points; reject comments trim to 1..500 Unicode code points; Long IDs serialize as strings; repeated identical actions return 200 with no side effects; opposite or illegal actions return 409/code `43000`.
