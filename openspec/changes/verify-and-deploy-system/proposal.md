...
- Record explicit local gates versus external gates. Standing user authorization is recorded for local verification commands in the dedicated worktree `D:\Projects\project1_campus\target\worktrees\verify-and-deploy-system`, limited to loopback targets (`127.0.0.1`, `localhost`, `::1`) and Docker/Docker Compose, Maven, npm, JMeter, headless Chrome, MySQL/Redis, migration, backup/restore, restart, and database checks. This does not authorize external hosts, DNS, TLS/certificates, public URLs, public deployment, or externally exposed 443; those remain separately gated. Without external infrastructure authorization, the change remains Draft and must not claim public deployment, domain, or certificate completion.
