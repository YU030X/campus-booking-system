...
- Add booking and `booking_slot` entities, annotation-based mappers, booking services, pure slot splitting, and booking-focused unit/integration/concurrency tests under the T07 ownership boundary only; follow the repository's existing inline MyBatis SQL style and do not add mapper XML resources.
...
- **Owned implementation area:** `booking-api/src/main/java/com/yu030x/booking/booking/**` and `booking-api/src/test/java/com/yu030x/booking/booking/**`, including `booking` and `booking_slot` entities, annotation-based mappers, transaction services, pure slot splitting, and concurrency tests.
