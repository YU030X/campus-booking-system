...
The API SHALL expose POST `/api/v1/auth/register` and `/api/v1/auth/login`. RegisterRequest and LoginRequest SHALL reject unknown JSON fields; validation SHALL match the design (including UTF-8 byte password length and trimming/null normalization). Registration returns HTTP 201 with `data: UserView` and no token. Login returns HTTP 200 `{token,tokenType:"Bearer",expiresIn:<actual TTL seconds>,user}` with integer seconds equal to exp-iat within clock precision; JWT_TTL_SECONDS 1..86400 is returned verbatim, default 7200. Passwords use BCrypt strength 12 and are never returned or logged.
